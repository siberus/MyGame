...
COLORREF CMyGameBoard::m_arrColors[8];
CMyGameBoard::CMyGameBoard(void)
	: m_arrBoard(NULL),
...
void CMyGameBoard::SetupBoard(void) 
{ 
...
COLORREF CMyGameBoard::GetBoardSpace(int row, int col)
{ 
...
void CMyGameBoard::DeleteBoard(void) 
{ 
	if (m_arrBoard != NULL) 
	{ 
		for (int row = 0; row < m_nRows; row++) 
...
void CMyGameBoard::CreateBoard(void) 
{ 
...
int CMyGameBoard::DeleteBlocks(int row, int col)
{
...
int CMyGameBoard::DeleteNeighborBlocks(int row, int col, int color,
	Direction direction)
{
	// Проверяем на корректность индексы ячейки и столбца 
	if (row < 0 || row >= m_nRows || col < 0 || col >= m_nColumns)
...
void CMyGameBoard::CompactBoard(void)
{
...
bool CMyGameBoard::IsGameOver(void) const
{
